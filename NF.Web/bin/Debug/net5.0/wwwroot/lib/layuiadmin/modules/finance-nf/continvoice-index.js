...
layui.define(['table', 'form', 'continvoiceutility', 'appflowutility', 'dynamicCondition', 'soulTable'], function (exports) {
   ...
   , continvoiceutility = layui.continvoiceutility
    ...
    var _reqUrl = '/Finance/ContInvoice/GetList?fType=1';
    ...
    table.render({
       ...
       , cols: [[
           ...
           , { field: 'InCode', title: '发票号', width: 150, filter: true, filter: true }
           , { field: 'CompName', title: '合同对方', width: 150, templet: '#compnameTpl', filter: true }
           , { field: 'AmountMoneyThod', title: '发票金额', width: 130, sort: true, filter: true }
       ...
       , limit: setter.table.limit
        ...
        , done: function (res, curr, count) {   //返回数据执行回调函数
           ...
           $("input[name=keyWord]").val($("input[name=hide_keyWord]").val());
            ...
            continvoiceutility.stateEvent({ tableId: 'NF-Invoice-Index' }, cate);//注册状态流转事件
    ...
    //监听表格排序
    table.on('sort(NF-Invoice-Index)', function (obj) {
        table.reload('NF-Invoice-Index', { //testTable是表格容器id
    ...
    var opencontract = function () {
        layer.open({
            type: 2
            , title: '选择合同'
            , content: '/NfCommon/SelectItem/SelectContract?selType=0'
            , maxmin: true
            , area: ['75%', '500px']
            //, btn: ['确定', '取消']
            , btnAlign: 'c'
    ...
    var active = {
        ...
        , batchdel: function () {//删除
            wooutil.deleteDatas({ tableId: 'NF-Invoice-Index', table: table, url: '/Finance/ContInvoice/Delete' });
        },
        search: function () {//查询
            $("input[name=hide_keyWord]").val($("input[name=keyWord]").val());
            table.reload('NF-Invoice-Index', {
                page: { curr: 1 }
                , where: {
            ...
            if (setter.sysinfo.seversion == "SE") {
                var checkStatus = table.checkStatus("NF-Invoice-Index")
                , checkData = checkStatus.data; //得到选中的数据
                var resf = appflowutility.showFlow({
                    tableId: 'NF-Invoice-Index'
                      , evtobj: evtobj
                      , objType: setter.sysWf.flowType.ShouPiao//收票
    ...
    table.on('toolbar(NF-Invoice-Index)', function (obj) {
        switch (obj.event) {
            case 'add':
                active.add();
                break;
            ...
            case "exportexcel":
                wooutil.exportexcel(obj, {
                    url: "/Finance/ContInvoice/ExportExcel?FType=1",
                    keyword: $("input[name=keyWord]").val()
                });
                break;
            case "advQuery"://高级查询
                ...
                break;
            case "clear":
                soulTable.clearCache("NF-Invoice-Index")
    ...
    **/
    function openview(obj) {
        layer.open({
            type: 2
                , title: '查看详情'
                , content: '/Finance/ContInvoice/DetailInvoice?Id=' + obj.data.Id + "&contId=" + obj.data.ContId+"&rand=" + wooutil.getRandom()
                , maxmin: true
                , area: ['60%', '80%']
                , btnAlign: 'c'
    ...
    **/
    function editFunc(obj, _success) {
        layer.open({
            type: 2
                , title: '修改发票'
            , content: '/Finance/ContInvoice/BuildInvoice?Id=' + obj.data.Id + "&contId=" + obj.data.ContId + "&rand=" + wooutil.getRandom()
                , maxmin: true
                , area: ['60%', '80%']
                //, btnAlign: 'c'
              ,success: function (layero, index) {
    ...
    table.on('tool(NF-Invoice-Index)', function (obj) {
        switch (obj.event) {
            case "del":
                {
                    var cate = layui.data(setter.tableName).userName;
                    if (obj.data.InState === 0 || cate == "SuperAdministrator") {
                        wooutil.deleteInfo({ tableId: "NF-Invoice-Index", data: obj, url: '/Finance/ContInvoice/Delete' });
                    } else {
            ...
            case "edit":
                {
                    ...
                    if (obj.data.InState === 0 || obj.data.InState==4) {//未提交Or被打回
                        var ress = getUpdatePermission("addOrUpdateInvoice",obj);
