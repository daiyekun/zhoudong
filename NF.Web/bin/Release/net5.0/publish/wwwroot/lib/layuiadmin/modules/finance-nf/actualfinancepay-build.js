...
layui.define(['table', 'form'], function (exports) {
    ...
    var tmpyh = decodeURI(wooutil.getUrlVar('yhName'));
    if (tmpyh == null || tmpyh == "null") {
        tmpyh = "";
    }
    $("#Bank").val(tmpyh);
    var tempzh = wooutil.getUrlVar('yhzh');
    if (tempzh == null || tempzh == "null") {
        tempzh = "";
    }
    $("#Account").val(tempzh);
             ...
             , done: function (res, curr, count) {
                 if (count > 0) {
                     ...
                     for (var i = 0; i < count; i++) {
                         var hxje = parseFloat(res.data[i].CheckAmount);
    ...
    var checkplanfinanceEvent = {
        toolchkplanfinance: function (obj) {
            ...
            if (DcontAmount === 0) {
                ...
                _va1 = dcmSub(ssd, DcontAmount);//(ssd - DcontAmount);
                if (_va1 < 0) {
                    _va1 = Math.abs(_va1);
                    syje = 0;
                    obj.update({
                    ...
                    _va2 = dcmSub(person(ssd), _va1); //(person(ssd) - _va1);
                    if (_va2 < DcontAmount) {
                        _va2 = 0;
                    }
                    obj.update({
    ...
    table.render({
        ...
        //, defaultToolbar: ['filter']
         ...
         , done: function (res, curr, count) {
             for (var i = 0; i < count; i++) {
                 totalamount = parseFloat(totalamount + parseFloat(res.data[i].AmountMoney)).toFixed(2);
             }
    ...
    var active = {
        save: function () {
            ...
            form.on('submit(NF-ActFinance-FormSubmit)', function (obj) {
                ...
                //核销计划资金剩余金额
                var ShenYHeXiaoJinE = parseFloat(wooutil.parseThousandsSeparator($("input[name=SYHXJE]").val()));
                //核销发票剩余金额
               // var ShenYHeXiaoFpJinE = parseFloat(wooutil.parseThousandsSeparator($("input[name=HXFPSYHXJE]").val()));
                var totalActMoney = parseFloat(totalamount + currmonery).toFixed(2);
                ...
                if (_IsFramework === 0) {
                    ...
                    if (CheckType === "0" && currmonery !== parseFloat(totalmonery)) {
                       ...
                       // layer.alert(JSON.stringify(checklist));
                        wootool.submit({ url: _url, data: { info: obj.field, chkData: checklist }, msg: '保存成功', index: $index, tableId: 'NF-ActualFinancePay-Index' });
                    }
                }
                else {

                    if (_IsFramework === 1) {//框架合同
                        wootool.submit({ url: _url, data: { info: obj.field, chkData: checklist }, msg: '保存成功', index: $index, tableId: 'NF-ActualFinancePay-Index' });
                    } else {
                        if (totalActMoney <= contAmount) {
                            var CheckType = $("#CheckType").val();
                            ...
                            if (CheckType === "0" && currmonery !== totalmonery) {
                                ...
                                //layer.alert(JSON.stringify(checklist));
                                wootool.submit({ url: _url, data: { info: obj.field, chkData: checklist }, msg: '保存成功', index: $index, tableId: 'NF-ActualFinanceColl-Index' });
                            }
                        }
                        else {
