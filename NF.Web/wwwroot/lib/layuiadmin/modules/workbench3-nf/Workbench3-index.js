...
layui.define(['table', 'desc3utility', 'form', 'appflowutility', 'dynamicCondition', 'treeSelect', 'soulTable'], function (exports) {
    var $ = layui.$
        , table = layui.table
        , setter = layui.setter
        , admin = layui.admin
        , desc3utility = layui.desc3utility
        , form = layui.form
    ...
    var _reqUrl = "/Workbench/Mydesk/GetList?contId=" + 4 + "&rand=" + wooutil.getRandom();
    ...
    //列表
    table.render({
        elem: '#NF-Workbench3-Index'
        , url: _reqUrl// '/Workbench/Mydesk/GetList?contId=' + 4 + '&rand=' + wooutil.getRandom()
        , toolbar: '#toolWork3'
        ...
        , done: function (res, curr, count) {   //返回数据执行回调函数
           ...
           desc3utility.stateEvent({ tableId: 'NF-Workbench3-Index' });//注册状态流转事件
    ...
    //监听表格排序
    table.on('sort(NF-Workbench3-Index)', function (obj) {
        table.reload('NF-Workbench3-Index', { //testTable是表格容器id
    ...
    var active = {
        ...
        batchdel: function () {//删除
            wooutil.deleteDatas({ tableId: 'NF-Workbench3-Index', table: table, url: '/Contract/ContractCollection/Delete' });
        },
        search: function () {//查询
            $("input[name=hide_keyWord]").val($("input[name=keyWord]").val());
            table.reload('NF-Workbench3-Index', {
                page: { curr: 1 }
                , where: {
        ...
        submitState: function (evtobj) {//提交状态
            var checkStatus = table.checkStatus("NF-Workbench3-Index")
    ...
    table.on('toolbar(NF-Workbench3-Index)', function (obj) {
        switch (obj.event) {
            case 'add':
                active.add();
                break;
            ...
            case "exportexcel":
                wooutil.exportexcel(obj, {
                    url: '/Workbench/Mydesk/ExportExcel?contId=' + 4 + '&rand=' + wooutil.getRandom(),
                    keyword: $("input[name=keyWord]").val()
                });
                break;
            case "advQuery"://高级查询
                ...
                break;
            case "clear"://高级查询
                soulTable.clearCache("NF-Workbench3-Index")
        ...
        layer.open({
            ...
            , btn1: function (index, layero) {
                ...
                if (obj.data.ContState === 0 && obj.data.ModificationTimes === 0) {
                    ...
                    wooutil.deleteInfo({ tableId: "NF-Workbench3-Index", data: obj, url: '/Contract/ContractCollection/Delete', success: suc });
   ...
   **/
    ...
    table.on('tool(NF-Workbench3-Index)', function (obj) {
        switch (obj.event) {
            case "del":
                {
                    if (obj.data.ContState === 0 && obj.data.ModificationTimes === 0) {
                        wooutil.deleteInfo({ tableId: "NF-Workbench3-Index", data: obj, url: '/Contract/ContractCollection/Delete' });
                    } else {
                        layer.alert("只有未执行且没有变更过的合同才允许删除！");
                        return false;
                    }
