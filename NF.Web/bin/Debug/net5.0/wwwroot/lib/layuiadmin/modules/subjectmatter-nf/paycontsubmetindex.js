...
layui.define(['table', 'nfBcTree', 'dynamicCondition', 'soulTable'], function (exports) {
   ...
   , admin = layui.admin
    ...
    //列表
    table.render({
        elem: '#NF-PaySubMetIndex'
        , url: '/Contract/ContSubjectMatter/GetMainList?fType=1&rand=' + wooutil.getRandom()
        , toolbar: '#toolPaySubMet'
        ...
        , cols: [[
            ...
            , { field: 'CompName', title: '合同对方', width: 180, templet: '#compTpl', filter: true}
            ...
            , { title: '操作', width: 180, align: 'center', fixed: 'right', toolbar: '#table-PaySubMetIndex-bar' }
        ]]
        , page: true
        , loading: true
    ...
    //监听表格排序
    table.on('sort(NF-PaySubMetIndex)', function (obj) {
        table.reload('NF-PaySubMetIndex', { //testTable是表格容器id
    ...
    var active = {
        search: function () {//查询
            table.reload('NF-PaySubMetIndex', {
                page: { curr: 1 }
                , where: {
        ...
        cateSearch: function () {//类别查询
            nfBcTree.showTree({ el: '#cateDiv', tableId: 'NF-PaySubMetIndex', treeId: 'bccateTree', hideFild: '#hdselCateIds' });
        },
        ...
        jiaoFu: function () {//交付
            var checkStatus = table.checkStatus('NF-PaySubMetIndex');
            ...
            parent.layui.index.openTabsPage("/Contract/ContSubDelivery/PaymentIndex?subIds=" + selIds, "付款标的交付明细");
        }
    };
    ...
    table.on('toolbar(NF-PaySubMetIndex)', function (obj) {
        switch (obj.event) {
            case 'search':
                active.search();
                break;
            case 'cateSearch'://类别查询
                ...
                break;
            case 'clear'://类别查询
                soulTable.clearCache("NF-PaySubMetIndex")
                layer.msg('已还原！', { icon: 1, time: 1000 })
                break;
            case "exportexcel":
                wooutil.exportexcel(obj, { url: "/Contract/ContSubjectMatter/ExportExcel?FType=1&cateIds=" + $("#hdselCateIds").val() });
                break;
            case "jiaoFu"://交付
                active.jiaoFu();
                break;
    ...
    table.on('tool(NF-PaySubMetIndex)', function (obj) {
        switch (obj.event) {
            case "jiaofu":
                {
