...
layuiXtree.prototype.Initial = function (o) {
    ...
    _this._ckall = o.ckall != null ? o.ckall : false;  //全选是否启用
     //console.log("传入值：" + o.chidparent);
    _this._chidparent = o.chidparent != null ? o.chidparent : true;  //子类选中，父类是否选中，默认开启
...
layuiXtree.prototype.Rendering = function () {
    ...
    _this._domStr = "";
   // console.log("是否父节点:" + _this._chidparent)
    //检查选中状态
    var xtree_ckitems = _this.getByClassName('layui-xtree-checkbox');
    for (var i = 0; i < xtree_ckitems.length; i++) {//_this._chidparent 父节点是否需要根据子节点转中其中一个就被选中
        if (xtree_ckitems[i].getAttribute('data-xend') == '1' && xtree_ckitems[i].checked && _this._chidparent) {
