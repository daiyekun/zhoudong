...
layui.define(['table', 'contractutility', 'form', 'appflowutility', 'dynamicCondition', 'treeSelect', 'soulTable'], function (exports) {
   ...
   , appflowutility = layui.appflowutility
    ...
    var _reqUrl = "/Inquiry/Inquiry/GetList?rand=" + wooutil.getRandom();
    ...
    var $htcols = [
        ...
        , { field: 'DeptName', title: '询价部门', width: 240, templet: '#nameTpl', fixed: 'left', sort: true }
    ...
    //列表
    table.render({
        elem: '#NF-InquiryCollection-Index'
       , url: _reqUrl
       ...
       , limit: setter.table.limit
        ...
        , done: function (res, curr, count) {   //返回数据执行回调函数
           ...
           contractutility.stateEvent({ tableId: 'NF-InquiryCollection-Index' });//注册状态流转事件

       }

    });
    //监听表格排序
    table.on('sort(NF-InquiryCollection-Index)', function (obj) {
        table.reload('NF-InquiryCollection-Index', { //testTable是表格容器id
            ...
            , yes: function (index, layero) {
                ...
                iframeWindow.layui.form.on('submit(' + submitID + ')', function (obj) {
                    ...
                    var resname = wooutil.UniqueValObj({
                        url: '/Inquiry/Inquiry/CheckInputValExist',
                    ...
                    var $url ="/Inquiry/Inquiry/Save";
                    if($currId>0){
                        $url = "/Inquiry/Inquiry/UpdateSave";
                    }
                    wooutil.OpenSubmitForm({
                        ...
                        msg: '保存成功',
                        index: index,
                        tableId: 'NF-InquiryCollection-Index'
                    });
                    return false;
                });
    ...
    var active = {
        ...
        batchdel: function () {//删除
            wooutil.deleteDatas({ tableId: 'NF-InquiryCollection-Index', table: table, url: '/Inquiry/Inquiry/Delete' });
        },
        search: function () {//查询
            $("input[name=hide_keyWord]").val($("input[name=keyWord]").val());
            table.reload('NF-InquiryCollection-Index', {
                page: { curr: 1 }
                , where: {
        ...
        submitState: function (evtobj) {//提交状态
            var checkStatus = table.checkStatus("NF-InquiryCollection-Index")
                , checkData = checkStatus.data; //得到选中的数据
            var ress = wooutil.requestpremission({
                url: '/NfCommon/NfPermission/StateUpdate?rand=' + wooutil.getRandom(),
                data: {
                    FuncCode: 'updateInquirystate',
            ...
            if (ress.RetValue == 0) {
                if (setter.sysinfo.seversion == "SE") {
                    ...
                    var resf = appflowutility.showFlow({
                        ...
                        , objCateId: checkData[0].InquiryType//类别
                    ...
                    if (resf == -1) {
                        layer.confirm('没有匹配上流程是否直接修改状态？', { icon: 3, title: '提示信息' }, function (cfindex) {
                            contractutility.updateSate({
                                tableId: 'NF-InquiryCollection-Index'
                                , url: '/Inquiry/Inquiry/UpdateField'
                                , evtobj: evtobj
                            });
                            layer.close(cfindex);
    ...
    table.on('toolbar(NF-InquiryCollection-Index)', function (obj) {
        switch (obj.event) {
            case 'add':
                active.add();
                break;
            ...
            case "exportexcel":
                wooutil.exportexcel(obj, {
                    url: "/Inquiry/Inquiry/ExportExcel",
                    keyword: $("input[name=keyWord]").val()
                });
                break;
            case 'clear'://状态流转
                soulTable.clearCache("NF-InquiryCollection-Index")
    ...
    function editFunc(obj, _success) {
        layer.open({
            type: 2
                , title: '修改信息'
            , content: '/Inquiry/Inquiry/Build?Id=' + obj.data.Id + "&rand=" + wooutil.getRandom()
                , maxmin: true
                , area: ['60%', '80%']
    ...
    function changeFunc(obj, _success) {
        layer.open({
            type: 2
                , title: '合同变更'
            , content: '/Inquiry/Inquiry/Change?Id=' + obj.data.Id + "&rand=" + wooutil.getRandom()
                , maxmin: true
                , area: ['60%', '80%']
            // , area: [window.screen.width / 2 + 'px', window.screen.height / 2 + 'px'] //宽高
                , btn: ['确定', '取消']
                ...
                , yes: function (index, layero) {
                    ...
                    iframeWindow.layui.form.on('submit(' + submitID + ')', function (obj) {
                        ...
                        wooutil.OpenSubmitForm({
                            url: '/Inquiry/Inquiry/ChangeSave',
                            table: table,
                            data: field,
                            tableId: 'NF-InquiryCollection-Index',
                            msg: '保存成功',
    ...
    **/
    function DetailBtnShowAndHide(obj) {
        admin.req({
            ...
            , data: { perCode: "Inquiry", Id: obj.data.Id }
           , done: function (res) {
    ...
    **/
    function openview(obj) {
        layer.open({
                  type: 2
                , title: '查看详情'
            , content: '/Inquiry/Inquiry/Detail?Id=' + obj.data.Id + "&rand=" + wooutil.getRandom()
                , maxmin: true
                , area: ['60%', '80%']
                 , btnAlign: 'c'
            ...
            , btn1: function (index, layero) {
                ...
                if (obj.data.InState === 0  ) {
                    ...
                    wooutil.deleteInfo({ tableId: "NF-InquiryCollection-Index", data: obj, url: '/Inquiry/Inquiry/Delete', success: suc });
   ...
   **/
    ...
    //        type: 2
    //            , title: '查看详情'
    //        , content: '/Inquiry/Inquiry/Detail?Id=' + obj.data.CompId + "&rand=" + wooutil.getRandom()
    //            , maxmin: true
    //            , area: ['60%', '80%']
    //            , btnAlign: 'c'
    //            , skin: "layer-nf-nfskin"
    ...
    table.on('tool(NF-InquiryCollection-Index)', function (obj) {
        switch (obj.event) {
            case "del":
                {
                    if (obj.data.InState === 0 || obj.data.InState === null) {
                        wooutil.deleteInfo({ tableId: "NF-InquiryCollection-Index", data: obj, url: '/Inquiry/Inquiry/Delete' });
                    } else {
                        layer.alert("执行中数据无法修改！");
                        return false;
                    }
