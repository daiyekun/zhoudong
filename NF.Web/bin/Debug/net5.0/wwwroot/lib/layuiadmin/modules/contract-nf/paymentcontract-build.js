...
layui.define(['table', 'form', 'subMetBuild', 'wordAddin', 'treeSelect', 'laydate', 'nfcontents', 'commonnf', 'tree', 'soulTable'], function (exports) {
    ...
    //监听分包合同Radio
    form.on('radio(ContDivision)', function (data) {
        if (data.value == 2) {
            $("input[name=SumContName]").removeClass("layui-bg-gray").removeClass("layui-disabled");
        } else {
            if (!$("input[name=SumContName]").hasClass("layui-disabled")) {
                $("input[name=SumContName]").val("");
                $("input[name=SumContId]").val("");
                $("input[name=SumContName]").addClass("layui-bg-gray").addClass("layui-disabled");
            }
        }
    });
    //预收金额
    ...
    **/
    ...
    var contTextEvent = {
        ...
        tempDraft: function (txtId) {
            ...
            var submitID = 'NF-ContractPayment-FormSubmit';
            var submit = $('#' + submitID);
