...
layui.define(['table', 'form'], function (exports) {
    ...
    var tenderutility = {
        stateEvent: function (param) {
            ...
            $(".downpanel").on("click", ".layui-select-title", function (e) {
                ...
                } else if (checkData[0].TenderStatusId==1||checkData[0].TenderStatusId == 2) {//已终止
                    return layer.msg('当前状态不允许修改！');
                }
                else {
                    $(".datastate").empty();
                    if (checkData[0].TenderStatusId == 0) {
                        $(".datastate").append('<dd lay-event="stateChange" flowitem="1" tostate="1"><i class="layui-icon layui-icon-edit"></i>未执行-->执行中</dd>');

                    } else if (checkData[0].TenderStatusId == 1) {//不会执行
        ...
        updateSate: function (param) {
            ...
            admin.req({
                url: param.url
                , data: { Id: checkData[0].Id, fieldName: 'TenderStatus', fieldVal: state }
                , done: function (res) {
    ...
    exports('tenderutility', tenderutility);
});