...
layui.define(['table', 'form'], function (exports) {
    ...
    var actfinaniceutility = {
        stateEvent: function (param,name) {
            ...
            $(".downpanel").on("click", ".layui-select-title", function (e) {
                ...
                if (checkData.length !== 1) {
                ...
                } else if (checkData[0].Astate == 2 && name != "SuperAdministrator") {//已终止
                    ...
                    if (name == "SuperAdministrator") {
                        ...
                        $(".datastate").append('<dd lay-event="stateChange" flowitem="2" tostate="2"><i class="layui-icon layui-icon-edit"></i>已确认</dd>');
                        $(".datastate").append('<dd lay-event="stateChange" flowitem="3" tostate="3"><i class="layui-icon layui-icon-edit"></i>被打回</dd>');
                        $(".datastate").append('<dd lay-event="stateChange" flowitem="6" tostate="6"><i class="layui-icon layui-icon-edit"></i>审批通过</dd>');
                    ...
                    $(".datastate").empty();
                    if (checkData[0].Astate == 0) {
                        var result = 0;
                        admin.req({
                            async: false,//取消异步
                            url: "/Finance/ContActualFinance/CheckContAmount",
                        ...
                        if (result === 1) {
                            return  layer.alert('已提交金额与本次提交金额之和不能大于合同金额！', { icon: 5 });
                    ...
                    } else if (checkData[0].Astate == 1) {
                        $(".datastate").append('<dd lay-event="stateChange" flowitem="2" tostate="2"><i class="layui-icon layui-icon-edit"></i>已提交-->已确认</dd>');
                        $(".datastate").append('<dd lay-event="stateChange" flowitem="3" tostate="3"><i class="layui-icon layui-icon-edit"></i>已提交-->被打回</dd>');
                    }
        ...
        updateSate: function (param) {
            ...
            if (state == "1") {
                ...
                admin.req({
                    url: param.url
             , data: { Id: checkData[0].Id, OtherId: checkData[0].ContId, FieldName: 'Astate', FieldValue: state, UpdateMoney: checkData[0].AmountMoney }
             , done: function (res) {
    ...
    exports('actfinaniceutility', actfinaniceutility);
});