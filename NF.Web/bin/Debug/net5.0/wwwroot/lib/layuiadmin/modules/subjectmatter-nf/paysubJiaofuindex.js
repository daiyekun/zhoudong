...
layui.define(['table', 'nfBcTree', 'laydate', 'soulTable'], function (exports) {
   ...
   , admin = layui.admin
    ...
    //列表
    table.render({
        elem: '#NF-PaySubMetMxIndex'
        , url: '/Contract/ContSubDelivery/GetMainList?fType=1&subIds=' + subIds +'&rand=' + wooutil.getRandom()
        ...
        , cols: [[
            ...
            , { field: 'CompName', title: '合同对方', templet: '#compTpl',width: 160, filter: true }
    ...
    //监听表格排序
    table.on('sort(NF-PaySubMetMxIndex)', function (obj) {
        table.reload('NF-PaySubMetMxIndex', { //testTable是表格容器id
    ...
    var active = {
        search: function () {//查询
            table.reload('NF-PaySubMetMxIndex', {
                page: { curr: 1 }
                , where: {
        ...
        cateSearch: function () {//类别查询
            nfBcTree.showTree({ el: '#cateDiv', tableId: 'NF-PaySubMetMxIndex', treeId: 'bccateTree', hideFild: '#hdselCateIds' });
    ...
    table.on('toolbar(NF-PaySubMetMxIndex)', function (obj) {
        switch (obj.event) {
            case 'search':
                active.search();
                break;
            ...
            case "exportexcel":
                wooutil.exportexcel(obj, {
                    url: "/Contract/ContSubDelivery/ExportExcel?FType=1&cateIds="
                        + $("#hdselCateIds").val()
                        + "&KeyWord=" + $("input[name=keyWord]").val()
                        + "&beginData=" + $("input[name=StratTime]").val()
                        + "&endData=" + $("input[name=EndTime]").val()
    ...
    table.on('tool(NF-PaySubMetMxIndex)', function (obj) {
        switch (obj.event) {
            case "download"://下载
                active.download(obj);
                break;
