...
layui.define(['table', 'form'], function (exports) {
    ...
    var userinfoutility = {
            ...
            $(".downpanel").on("click", ".layui-select-title", function (e) {
                ...
                if (checkData.length !== 1) {
                    ...
                    if (checkData[0].State == 0) {
                        $(".datastate").append('<dd lay-event="stateChange" flowitem="1" tostate="1"><i class="layui-icon layui-icon-edit"></i>未启用-->启用</dd>');

                    } else if (checkData[0].State == 1) {
                        $(".datastate").append('<dd lay-event="stateChange" flowitem="2" tostate="2"><i class="layui-icon layui-icon-edit"></i>启用-->离职</dd>');
                    }
        ...
        updateSate:function(param) {
            ...
            admin.req({
             url: param.url
                , data: { Id: checkData[0].Id, fieldName: 'State', fieldValue: state }
           , done: function (res) {
    ...
    exports('userinfoutility', userinfoutility);
});