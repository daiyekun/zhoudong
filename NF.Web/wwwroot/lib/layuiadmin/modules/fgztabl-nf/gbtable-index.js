...
layui.define(['table', 'gbdescutility', 'form', 'appflowutility', 'dynamicCondition', 'treeSelect', 'soulTable'], function (exports) {
    var $ = layui.$
        , table = layui.table
        , setter = layui.setter
        , admin = layui.admin
        , gbdescutility = layui.gbdescutility
        , form = layui.form
    ...
    var _reqUrl = "/Workbench/Gzdesk/GetList?contId=" + 4 + "&rand=" + wooutil.getRandom();
    ...
    //列表
    table.render({
        elem: '#NF-Gbtable-Index'
        , url: _reqUrl//'/Workbench/Gzdesk/GetList?contId=' + 4+ '&rand=' + wooutil.getRandom()
        , toolbar: '#toolWork3'
        ...
        , done: function (res, curr, count) {   //返回数据执行回调函数
            ...
            gbdescutility.stateEvent({ tableId: 'NF-Gbtable-Index' });//注册状态流转事件
    ...
    //监听表格排序
    table.on('sort(NF-Gbtable-Index)', function (obj) {
        table.reload('NF-Gbtable-Index', { //testTable是表格容器id
    ...
    var active = {
        ...
        submitState: function (evtobj) {//提交状态
            var checkStatus = table.checkStatus("NF-Gbtable-Index")
                    ...
                    if (resf === -1) {
                        layer.confirm('没有匹配上流程是否直接修改状态？', { icon: 3, title: '提示信息' }, function (cfindex) {
                            descutility.updateSate({
                                tableId: 'NF-Gbtable-Index'
                                , url: '/Workbench/Gzdesk/UpdateMoreField'
                                , evtobj: evtobj
                            });
                            layer.close(cfindex);
                        });
        ...
        updstatec: function (evtobj) {//提交状态
            ...
            gbdescutility.updateSate({
                tableId: 'NF-Gbtable-Index'
                , url: '/Workbench/Gzdesk/Updstate?contId=' + id + '&ty=' + 1 + '&rand=' + wooutil.getRandom()
                , evtobj: evtobj
            });
        ...
        updstateg: function (evtobj) {//提交状态
            ...
            gbdescutility.updateSate({
                tableId: 'NF-Gbtable-Index'
                , url: '/Workbench/Gzdesk/Updstate?contId=' + id + '&ty=' + 2 + '&rand=' + wooutil.getRandom()
                , evtobj: evtobj
            });
    ...
    table.on('toolbar(NF-Gbtable-Index)', function (obj) {
        switch (obj.event) {
            case 'add':
                active.add();
                break;
            ...
            case "exportexcel":
                wooutil.exportexcel(obj, {
                    url: '/Workbench/Gzdesk/ExportExcel?contId=' + 4 + '&rand=' + wooutil.getRandom(),
                    keyword: $("input[name=keyWord]").val()
                });
                break;
            case "advQuery"://高级查询
                ...
                break;
            case "clear"://高级查询
                soulTable.clearCache("NF-Gbtable-Index")
    ...
    **/
    ...
    table.on('tool(NF-Gbtable-Index)', function (obj) {
        switch (obj.event) {
            case "del":
                {
                    if (obj.data.ContState === 0 && obj.data.ModificationTimes === 0) {
                        wooutil.deleteInfo({ tableId: "NF-Gbtable-Index", data: obj, url: '/Contract/ContractCollection/Delete' });
                    } else {
