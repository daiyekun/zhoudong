...
namespace NF.WeiXinApp.Utility.Common
{
    public static class FileStreamingHelper
    {
        ...
        public static async Task<FormValueProvider> StreamFiles_Video(this HttpRequest request, string targetDirectory, EnterpriseFile uploadFileInfo)
        {
            if (!MultipartRequestHelper.IsMultipartContentType(request.ContentType))
            {
                throw new Exception($"Expected a multipart request, but got {request.ContentType}");
        ...
        public static async Task<FormValueProvider> StreamFiles_Video(this HttpRequest request, string targetDirectory, CheckFile uploadFileInfo)
        {
            if (!MultipartRequestHelper.IsMultipartContentType(request.ContentType))
            {
                throw new Exception($"Expected a multipart request, but got {request.ContentType}");
