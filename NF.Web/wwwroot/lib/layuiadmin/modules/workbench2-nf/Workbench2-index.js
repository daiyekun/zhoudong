...
layui.define(['table', 'desc2utility', 'form', 'appflowutility', 'dynamicCondition', 'treeSelect', 'soulTable'], function (exports) {
    var $ = layui.$
        , table = layui.table
        , setter = layui.setter
        , admin = layui.admin
        , desc2utility = layui.desc2utility
        , form = layui.form
    ...
    var _reqUrl = "/Workbench/Mydesk/GetList?contId=" + 3 + "&rand=" + wooutil.getRandom();
    ...
    //列表
    table.render({
        elem: '#NF-Workbench2-Index'
        , url: _reqUrl// '/Workbench/Mydesk/GetList?contId=' + 3 + '&rand=' + wooutil.getRandom()
        ...
        , done: function (res, curr, count) {   //返回数据执行回调函数
            ...
            desc2utility.stateEvent({ tableId: 'NF-Workbench2-Index' });//注册状态流转事件
    ...
    //监听表格排序
    table.on('sort(NF-Workbench2-Index)', function (obj) {
        table.reload('NF-Workbench2-Index', { //testTable是表格容器id
    ...
    var openAdd = function () {
        layer.open({
            type: 2
            , title: '新增合同'
            , content: '/Contract/ContractCollection/Build'
            , maxmin: true
            , area: ['60%', '80%']
            , btn: ['确定', '取消']
            , btnAlign: 'c'
    ...
    var active = {
        ...
        batchdel: function () {//删除
            wooutil.deleteDatas({ tableId: 'NF-Workbench2-Index', table: table, url: '/Contract/ContractCollection/Delete' });
        },
        search: function () {//查询
            $("input[name=hide_keyWord]").val($("input[name=keyWord]").val());
            table.reload('NF-Workbench2-Index', {
                page: { curr: 1 }
                , where: {
        ...
        submitState: function (evtobj) {//提交状态
            var checkStatus = table.checkStatus("NF-Workbench2-Index")
    ...
    table.on('toolbar(NF-Workbench2-Index)', function (obj) {
        switch (obj.event) {
            case 'add':
                active.add();
                break;
            ...
            case "exportexcel":
                wooutil.exportexcel(obj, {
                    url: '/Workbench/Mydesk/ExportExcel?contId=' + 3 + '&rand=' + wooutil.getRandom(),
                    keyword: $("input[name=keyWord]").val()
                });
                break;
            case "advQuery"://高级查询
                ...
                break;
            case "clear"://高级查询
                soulTable.clearCache("NF-Workbench2-Index")
    ...
    function editFunc(obj, _success) {
        layer.open({
            type: 2
            , title: '修改信息'
            , content: '/Contract/ContractCollection/Build?Id=' + obj.data.Id + "&rand=" + wooutil.getRandom()
            , maxmin: true
            , area: ['60%', '80%']
    ...
    function changeFunc(obj, _success) {
        layer.open({
            type: 2
            , title: '合同变更'
            , content: '/Contract/ContractCollection/Change?Id=' + obj.data.Id + "&rand=" + wooutil.getRandom()
            , maxmin: true
            , area: ['60%', '80%']
            // , area: [window.screen.width / 2 + 'px', window.screen.height / 2 + 'px'] //宽高
            , btn: ['确定', '取消']
            ...
            , yes: function (index, layero) {
                ...
                iframeWindow.layui.form.on('submit(' + submitID + ')', function (obj) {
                    ...
                    wooutil.OpenSubmitForm({
                        url: '/Contract/ContractCollection/ChangeSave',
                        table: table,
                        data: field,
                        tableId: 'NF-Workbench2-Index',
                        msg: '保存成功',
        ...
        layer.open({
            ...
            , btn1: function (index, layero) {
                ...
                if (obj.data.ContState === 0 && obj.data.ModificationTimes === 0) {
                    ...
                    wooutil.deleteInfo({ tableId: "NF-Workbench2-Index", data: obj, url: '/Contract/ContractCollection/Delete', success: suc });
   ...
   **/
    ...
    table.on('tool(NF-Workbench2-Index)', function (obj) {
        switch (obj.event) {
            case "del":
                {
                    if (obj.data.ContState === 0 && obj.data.ModificationTimes === 0) {
                        wooutil.deleteInfo({ tableId: "NF-Workbench2-Index", data: obj, url: '/Contract/ContractCollection/Delete' });
                    } else {
                        layer.alert("只有未执行且没有变更过的合同才允许删除！");
                        return false;
                    }
