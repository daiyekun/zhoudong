...
layui.define(['form', 'laydate', 'commonnf', "viewPageEdit", 'appListHist', 'table'], function (exports) {
    ...
    layui.use('table', function () {
       ...
       **/
        var tabdesc = table.render({
            elem: '#NF-Invoice-desc'
             ...
             , cols: [[
                 ...
                 , { field: 'Count', title: '数量', width: 160 }
        ...
        **/
        function editFunc() {
            var ress = getUpdatePermission("addOrUpdateInvoiceOut");
            if (ress.RetValue == 0) {
                parent.layer.title("修改发票", $index);
                parent.layer.iframeSrc($index, '/Finance/ContInvoice/BuildInvoiceOut?Id=' + Id + "&contId=" + contId + "&rand=" + wooutil.getRandom());
            } else {
        ...
        function updateState(state) {
            admin.req({
                url: '/Finance/ContInvoice/UpdateField'
                   ...
                   , done: function (res) {
                       wootool.handlesucc({ msg: '操作成功！', time: 400, index: $index, tableId: 'NF-InvoiceOut-Index' });
                   }
            });
        }
        /**
        ...
        **/
        var active = {
            edit: function () {
                ...
                var tempId = [];
                tempId.push(Id);
                wootool.del({ url: '/Finance/ContInvoice/Delete', Ids: tempId, index: $index, tableId: 'NF-InvoiceOut-Index' });
            }, submit: function () {//提交
                var ress = getUpdatePermission("addOrUpdateInvoiceOut");
                ...
                var ress = getUpdatePermission("ConfirmOrBackInvoiceOut");
                if (ress.RetValue == 0) {
                    updateState(2);
                } else {
                    return layer.alert("无权限");
...
/***********************附件信息-end***************************************************************************************************/
    ...
    //审批历史
    appListHist.applistInit({ Id: Id, objType: setter.sysWf.flowType.KaiPiao });


    exports('continvoiceOutDetail', {});
});